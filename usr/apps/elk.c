...
static size_t strnum(jsval_t value, char *buf, size_t len) {
  double dv = tod(value);
  sys->sprintf(buf, "%d", (int)dv);
  return sys->strlen(buf);
}
...
jsval_t js_mkerr(struct js *js, const char *xx, ...) {
  ...
  sys->strncpy(js->errmsg + n, xx, sizeof(js->errmsg) - n - 1);
...
static size_t tostr(struct js *js, jsval_t value, char *buf, size_t len) {
  switch (vtype(value)) {  // clang-format off
    ...
    case T_CFUNC: sys->sprintf(buf, "\"c_func_0x%x\"", (unsigned int) vdata(value)); return sys->strlen(buf);
    case T_PROP:  sys->sprintf(buf, "PROP@%u", (unsigned int) vdata(value)); return sys->strlen(buf);
    default:      sys->sprintf(buf, "VTYPE%d", (int)vtype(value)); return sys->strlen(buf);
  }  // clang-format on
}
